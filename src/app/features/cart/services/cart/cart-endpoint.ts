...
export class CartEndpoint {
    ...
    getStoreProduct(store: any, requestStateUpdater: StoreRequestStateUpdater): Promise<Product>{
        const request = CART_CONFIG.request.getStoreProduct;
        requestStateUpdater(request.name, {inProgress: true});
    ...
    subtractStockAvalaible(product: any, requestStateUpdater: StoreRequestStateUpdater): Promise<Product>{
        const request = CART_CONFIG.request.subtractStockAvailable;
        requestStateUpdater(request.name, {inProgress: true});
    ...
    addStockAvalaible(product: any, requestStateUpdater: StoreRequestStateUpdater): Promise<Product>{
        const request = CART_CONFIG.request.addStockAvailable;
        requestStateUpdater(request.name, {inProgress: true});
        return this.apiService.put<Product>(request.path, product)
        .pipe(
            tap(
